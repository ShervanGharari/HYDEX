# this is funcion reads ts3 files format that is generated by ECCC explorer from the HYDAT data set and convert them into a
# NetCDF file that contains the values (of flow), flags, time, name and ID of the stations. The code can be used to be coupled
# with any other open source code that reads HYDAT data as a data frame, or read the text or csv files.
# this code is under Apache-2 form Shervan Gharari's Personal library of codes and functions (sh.gharari@gmail.com).
# there is no waranty and liability for using this code.
